...
def parse(s):
    if matches := re.search(r'src="https?://(www\.)?youtube\.com/embed/([^"]+)"', s): # ([^"]+) captures one or more characters that are not (")
        return f"https://youtu.be/{matches.group(2)}" # group(1) is (www\.)? and group(2) is ([^"]+)
    return None
